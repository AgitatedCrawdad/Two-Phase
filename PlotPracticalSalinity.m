...
if exist('V190')==1
    ...
    SP = @(a) gsw_SP_from_C(a.C1,a.T4,a.P1*0.689476);
...
%     plt = errorbar(x,y,errors,'-o','LineWidth',2);
    ...
    x = x(1:end-1);
    SP = @(a) gsw_SP_from_C(a.C1,a.T5,a.P1*0.689476);
    sp1 = SP(a); sp2=SP(b); sp3=SP(c); sp4=SP(d); sp5=SP(e); sp6=SP(f); sp7=SP(g); sp8=SP(h);
    plot(x(1),sp1,x(2),sp2);
    disp(a.P1)
    error_a = std(sp1); error_b = std(sp2);error_c = std(sp3);error_d = std(sp4);
    error_e = std(sp5);
    error_f = std(sp6); error_g = std(sp7);error_h = std(sp8);

    y = [mean(sp1) mean(sp2) mean(sp3) mean(sp4) mean(sp5) mean(sp6) mean(sp7) mean(sp8)];
%     error_a = std(get(a,prop)); error_b = std(get(b,prop));error_c = std(get(c,prop));error_d = std(get(d,prop));
%     error_e = std(get(e,prop));
