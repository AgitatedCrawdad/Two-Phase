classdef Temp
   ...
   methods
      ...
      function r = temps(obj) %This function is used to plot the unfiltered thermocouple(TC) data
          figure
          r = plot(obj.Time,obj.T1,obj.Time,obj.T2,obj.Time,obj.T3);
          legend('T1', 'T2', 'T3');
          axes3=gca;
      ...
      function r = smoothtemps(obj) %This function is used to plot the filtered TC data using a moving average
          figure;
          r = plot(obj.Time(1:length(obj.T1S)),obj.T1S,obj.Time(1:length(obj.T1S)),obj.T2S,obj.Time(1:length(obj.T1S)),obj.T3S);
          legend('T1','T2','T3')
          axes3=gca;
          set(axes3,'FontName','Times New Roman','FontSize',28,'Layer','top','LineWidth',3);
          xlabel('Time [s]')
